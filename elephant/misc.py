...
def generate_data(lambda_b,lambda_c,T,T_h,nTrials,RateJitter = 0*pq.Hz):
    ...
    for tr in range(nTrials):
        ...
        s1 = numpy.sort(numpy.append(sp_interval2_tmp[0].rescale('ms').magnitude,coinc_sp.rescale('ms').magnitude))
        s2 = numpy.sort(numpy.append(sp_interval2_tmp[1].rescale('ms').magnitude,coinc_sp.rescale('ms').magnitude))
        ...
        spiketrain1 = numpy.append(
            sp_interval1[0].rescale('ms').magnitude ,sp_interval2[0].rescale('ms').magnitude)
        spiketrain1 = numpy.append(
            spiketrain1, sp_interval3[0].rescale('ms').magnitude)

        spiketrain2 = numpy.append(
            sp_interval1[1].rescale('ms').magnitude, sp_interval2[1].rescale('ms').magnitude)
        spiketrain2 = numpy.append(
...
def _plot_UE(data,Js_dict,sig_level,binsize,winsize,winstep, pattern_hash,N,args,add_epochs = []):
    ...
    for n in range(N):
        for tr,data_tr in enumerate(data):
            plt.plot(data_tr[n].rescale('ms').magnitude, numpy.ones_like(data_tr[n].magnitude)*tr + n*(num_tr + 1) + 1, ',',color = 'k')
