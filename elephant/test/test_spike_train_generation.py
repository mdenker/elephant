...
class AnalogSignalThresholdDetectionTestCase(unittest.TestCase):
    ...
    def test_threshold_detection(self):
        ...
        spike_train = stgen.threshold_detection(vm)[0]
        try:
            len(spike_train)
        except TypeError:  # Handles an error in Neo related to some zero length
            # spike trains being treated as unsized objects.
            warnings.warn((
                          "The spike train may be an unsized object. This may "
                          "be related to an issue in Neo with some zero-length "
                          "SpikeTrain objects. Bypassing this by creating an "
                          "empty SpikeTrain object."))
    ...
    def test_multidim_analogsignal(self):
        anasig = neo.AnalogSignal(np.array([
            ...
            np.tan(np.arange(0, 20 * math.pi, 0.1))]).T,
                                  ...
                                  channel_id=2)
        # anasig = neo.AnalogSignal(np.array([np.arange(10),
        #                                     np.arange(10)/2]).T*mV,
        #                           t_start=0*ms,
        #                           sampling_rate=1*Hz)
        sts = stgen.threshold_detection(anasig, threshold=1 * mV)

        expected_crossings = np.array([0.0008, 0.004, 0.0071, 0.0103, 0.0134,
                                       0.0165, 0.0197, 0.0228, 0.026, 0.0291,
                                       0.0323, 0.0354, 0.0385, 0.0417, 0.0448,
                                       0.048, 0.0511, 0.0542, 0.0574, 0.0605])

        # only last signal trace exceeds threshold at all
        self.assertEqual(len(sts), 3)
        assert_array_almost_equal(sts[2].times.magnitude, expected_crossings)
...
class AnalogSignalPeakDetectionTestCase(unittest.TestCase):
    ...
    def test_peak_detection_time_stamps(self):
        ...
        result = stgen.peak_detection(self.vm)[0]
    ...
    def test_single_sample_below_threshold(self):
        anasig = neo.AnalogSignal(np.full((100), 1) * mV,
                                  t_start=0 * ms,
                                  sampling_rate=1 * kHz)
        anasig[50] = -1 * mV
        sts = stgen.peak_detection(anasig, threshold=0 * mV)
        self.assertEqual(len(sts), 1)
        self.assertEqual(sts[0], np.array([0]) * ms)
    ...
    def test_multidim_analogsignal(self):
        ...
        expected_peaks = np.array([0.0015, 0.0047, 0.0078, 0.0109, 0.0141,
                                   0.0172, 0.0204, 0.0235, 0.0267, 0.0298,
                                   0.0329, 0.0361, 0.0392, 0.0424, 0.0455,
                                   0.0486, 0.0518, 0.0549, 0.0581, 0.0612])
        sts = stgen.peak_detection(anasig, threshold=1 * mV)

        # only last signal trace exceeds threshold at all
        self.assertEqual(len(sts), 3)
        assert_array_almost_equal(sts[2].times.magnitude, expected_peaks)
    ...
    def test_statistics(self):
        ...
        for rate in [123.0 * Hz, 0.123 * kHz]:
            for t_stop in [2345 * ms, 2.345 * second]:
                spiketrain = stgen.homogeneous_poisson_process(rate,
                                                               t_stop=t_stop)
                intervals = isi(spiketrain)

                expected_spike_count = int((rate * t_stop).simplified)
                self.assertLess(pdiff(expected_spike_count, spiketrain.size),
                                0.2)  # should fail about 1 time in 1000

                expected_mean_isi = (1 / rate)
                self.assertLess(pdiff(expected_mean_isi, intervals.mean()), 0.2)

                expected_first_spike = 0 * ms
                self.assertLess(spiketrain[0] - expected_first_spike,
                                7 * expected_mean_isi)

                expected_last_spike = t_stop
                self.assertLess(expected_last_spike - spiketrain[-1],
                                7 * expected_mean_isi)
...
class HomogeneousGammaProcessTestCase(unittest.TestCase):
    ...
    def test_statistics(self):
        ...
        for b in (67.0 * Hz, 0.067 * kHz):
            for t_stop in (2345 * ms, 2.345 * second):
                spiketrain = stgen.homogeneous_gamma_process(a, b,
                                                             t_stop=t_stop)
                intervals = isi(spiketrain)

                expected_spike_count = int((b / a * t_stop).simplified)
                self.assertLess(pdiff(expected_spike_count, spiketrain.size),
                                0.25)  # should fail about 1 time in 1000
...
class _n_poisson_TestCase(unittest.TestCase):
    ...
    def test_poisson_error(self):
        ...
        # Negative n
        self.assertRaises(
                ValueError, stgen._n_poisson, rate=self.rate,
                t_stop=self.t_stop,
                n=-1)
        # t_start>t_stop
        self.assertRaises(
...
class singleinteractionprocess_TestCase(unittest.TestCase):
    ...
    def test_sip(self):
        ...
        sip = stgen.single_interaction_process(
                n=self.n, t_stop=self.t_stop, rate=self.rate,
                rate_c=self.rate_c, coincidences='stochastic',
                return_coinc=False)
...
class cppTestCase(unittest.TestCase):
    def test_cpp_hom(self):
        ...
        self.assertEqual(
                [len(train) for train in cpp_hom_empty],
                [0] * len(cpp_hom_empty))
