...
def poisson(rate, t_stop, t_start=0 * pq.s, N=1):
    """
...
def _analog_signal_step_interp(signal, times):
    ...
    return(signal.magnitude[time_ids] * signal.units).rescale(signal.units)[:,0]


def poisson_nonstat_thinning(rate, N=1):
    """
    ...
    Output
    -----
    Poisson SpikeTrain with profile rate lambda(t)= rate
    """
    ...
    # Define the interpolation method
        ...
        # Generate n hidden Poisson SpikeTrains with rate equal to the max rate
        lambda_star = np.max(rate)
        poiss = poisson(
            rate=lambda_star, t_stop=rate.t_stop,
            t_start=rate.t_start, N=N)
        ...
        for st in poiss:
            ...
            lamb = _analog_signal_step_interp(
                signal=rate, times=st.magnitude * st.units)
...
def poisson_nonstat_time_rescale(rate, N=1, csteps=1000):
    """
    ...
    # rescaling the unit of the signal
    elif np.any(rate > 0) and rate.units == pq.Hz:
        signal_simpl = rate.simplified
        t_start_simpl = rate.t_start.simplified
        t_stop_simpl = rate.t_stop.simplified
        sampling_period_simpl = rate.sampling_period.simplified
        rate = neo.AnalogSignal(
        ...
        for cn in range(N):
            ...
            st = neo.SpikeTrain(
                buf, t_stop=rate.t_stop - rate.t_start,
                units=rate.t_stop.units)
#            st = st + rate.t_start
            st = neo_tools.shift_spiketrain(st, rate.t_start)
            st.t_start = rate.t_start
            st.t_stop = rate.t_stop
            out.append(st)
        return out
    elif rate.units == pq.Hz:
        return(
            [neo.SpikeTrain(
                [], t_stop=rate.t_stop,
                units=rate.t_stop.units) for i in range(N)])
    else:
        raise ValueError(
            'rate must be in Hz, representing the rate at time t')


def poisson_nonstat(rate, N=1, method='time_rescale'):
    """
    Generates an ensemble of non-stationary Poisson processes with identical
    intensity.
    ...
    if isinstance(rate, neo.AnalogSignalArray):
        sts = []
        if rate.shape[1] == 1:
            sts = method_use(rate, N=N)
        else:
            for i in range(rate.shape[1]):
                sts = sts + method_use(rate[:, i])
    else:
        raise ValueError("Then input rate has to be a neo AnalogSignalArray")
    return sts


def sip(rate, rate_c, t_stop, N=2, jitter=0 * pq.s, tot_coinc='det',
        t_start=0 * pq.s, min_delay=0 * pq.s,
        return_coinc=False):
    """
    ...
    t_stop [Quantity]
        ...
        0 and t_stop. A time unit from the 'quantities' package can be
        assigned to t_stop (recommended)
    rate [Quantity]
        ...
        * an iterable of floats (one float per process), each float
          ...
          N entries must be larger than rate_c.
    rate_c [Quantity]
    ...
    --------
     output [list]
         realization of a SIP consisting of N Poisson processes characterized by
         ...
         unit is assigned to the output times (same as t_stop's. Default to sec).

          If return_coinc == True, the coincidence times are returned as a second
          output argument. They also have an associated time unit (same as t_stop's
          Default to sec).
    ...
    *************************************************************************
    ...
    if rate.ndim == 0:
        ...
        rates_b = np.array(
            [rate.magnitude for _ in xrange(N)]) * rate.units
    else:
    ...
    embedded_poisson_trains = [
        pq.Quantity(emb.base) * emb.units for emb in embedded_poisson_trains]
    ...
    if tot_coinc in ['det', 'd', 'deterministic']:
        Nr_coinc = int(((t_stop - t_start) * rate_c).rescale(pq.dimensionless))
        while 1:
            coinc_times = t_start + \
        ...
        while 1:
            coinc_times = poisson(
                rate=rate_c, t_stop=t_stop, t_start=t_start, N=1)[0]
            if len(coinc_times) < 2 or min(np.diff(coinc_times)) >= min_delay:
                break
...
def sip_nonstat(
        rate, rate_c, N=2, jitter=0 * pq.s, tot_coinc='det',
        start=0 * pq.s, min_delay=0 * pq.s, return_coinc=False):
    """
    Generates a multidimensional Poisson SIP (single interaction process)
    ...
    *************************************************************************
    ...
    if isinstance(rate, neo.AnalogSignalArray):
        ...
        # Check: rate_b>rate_c
        if np.min([np.min(rate) for r in rate]) < rate_c.rescale(
            rate[0].units).magnitude:
                raise ValueError(
                    'all elements of *rate_b* must be >= *rate_c*')
        ...
        # Generate the N Poisson processes there are the basis for the SIP
        # (coincidences still lacking) in the case of input one rate profile
        # and N number of neurons
        if rate.shape[1] == 1:
            embedded_poisson_trains = poisson_nonstat(rate=rate, N=N)
        else:
            embedded_poisson_trains = poisson_nonstat(rate=rate)
            N = rate.shape[1]
        # Convert the trains from neo SpikeTrain objects to simpler Quantity
        # objects
        embedded_poisson_trains = [
        ...
        # Generate the array of times for coincident events in SIP, not closer
        # than min_delay. The array is generated as a quantity from the
        # Quantity class in the quantities module
        T = rate.t_stop
        start = rate.t_start
        if tot_coinc in ['det', 'd', 'deterministic']:
            ...
            while 1:
                ...
                if len(
                    coinc_times) < 2 or min(np.diff(coinc_times)) >= min_delay:
                    break
        elif tot_coinc in ['s', 'stoc', 'stochastic']:
            while 1:
                coinc_times = poisson(
                ...
                if len(
                    coinc_times) < 2 or min(np.diff(coinc_times)) >= min_delay:
                    break
            # Convert coinc_times from a neo SpikeTrain object to a Quantity
            # object pq.Quantity(coinc_times.base)*coinc_times.units
            coinc_times = coinc_times.view(pq.Quantity)
            # Set the coincidence times to T-jitter if larger.This ensures that
            # the last jittered spike time is <T
            for i in range(len(coinc_times)):
        ...
        embedded_coinc = coinc_times + \
            np.random.random((N, len(coinc_times))) * 2 * jitter - jitter
        embedded_coinc = embedded_coinc + \
            (start - embedded_coinc) * (embedded_coinc < start) - \
            (T - embedded_coinc) * (embedded_coinc > T)

        # Inject coincident events into the SIP processes generated above, and
        # merge with the N independent processes
        sip_process = [
            np.sort(np.concatenate((
    ...
    *************************************************************************
    ...
    if M == 0:
        ...
        embedded_poisson_trains = poisson_nonstat(
            rate=[r - rate_c for r in rate_b[:M]])
        # Convert the trains from neo SpikeTrain objects to simpler Quantity
        # objects
        embedded_poisson_trains = [
            pq.Quantity(emb.base) * emb.units
...
def _mother_proc_cpp_nonstat(A, rate, method='time_rescale'):
    """
    ...
    rate : neo.core.AnalogSignal, units=Hz
        The analog signal containing the discretization on the time axis of the
        rate profile function of the spike trains to generate
    ...
    if exp_A == 0:
        ...
        rate_M = rate * N / float(exp_A)
    return method_use(
        rate=rate_M)[0]


def _cpp_hom_nonstat(A, rate, method='time_rescale'):
    """
    ...
    mother = _mother_proc_cpp_nonstat(A, rate, method=method)
...
def _cpp_het_nonstat(A, signals, method='time_rescale'):
    ...
    cpp = _cpp_hom_nonstat(A=a, rate=sign_min, method=method)
...
def cpp_nonstat(A, rate, method='time_rescale'):
    ...
    if type(rate) == neo.AnalogSignal:
        return _cpp_hom_nonstat(A=A, rate=rate, method=method)
    elif len(rate) == 1:
        return _cpp_hom_nonstat(A=A, rate=rate[0], method=method)
    else:
...
def gamma_thinning(t_stop, shape, rate, N=1, t_start=0*pq.s):
    """
...
def gamma_nonstat_rate(rate, shape, N=1):
    ...
    lambda(t)= rate and Gamma distribuited ISI
    """
    if type(shape) == int:
        if type(rate) == neo.AnalogSignal:
            ...
            rate_adj = neo.AnalogSignal(
                np.hstack(
                    (
                        rate[0].magnitude*np.array(10*[1]),
                        rate.magnitude)), units=rate.units,
                sampling_period=rate.sampling_period,
                t_start=rate.t_start -
                10 * rate.sampling_period) * shape
            t_start = [rate.t_start] * N
        elif all([type(r) == neo.AnalogSignal for r in rate]):
            rate_adj = [
                neo.AnalogSignal(
                    np.hstack(
                        (
                            r[0].magnitude*np.array(10*[1]), r.magnitude)),
            ...
            raise ValueError(
                'rate has to be an neo.AnalogSiagnal or a list of '
                'neo.AnalogSiagnal instead is %s' % type(rate))
        #Poisson non-stationary process to be thinned
        poiss = poisson_nonstat(rate=rate_adj, N=N)
...
def cgp_nonstat(A, shape, rate):
    ...
    proc_marg.append(
        gamma_nonstat_rate(rate=rate_marg[1], shape=shape, N=N))
    ...
    for i, j in enumerate(index[1:]):
        ...
        proc_marg.append(
            _cpp_hom_nonstat(
                A=amplitudes[i], rate=rate_marg[j]))
