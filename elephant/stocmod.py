...
def cpp(A, t_stop, rate, t_start=0 * pq.s, jitter=None):
    ...
    if jitter is None:
        ...
        cpp = [
            surr.dither_spike_train(
                cp, shift=jitter, edges=True)[0] for cp in cpp]
        return cpp
...
#            st = st + rate_signal.t_start
            st = neo_tools.shift_spiketrain(st, rate_signal.t_start)
            ...
            rate_adj = neo.AnalogSignal(
                np.hstack(
                    (
                        ...
                        rate_signal.magnitude)), units=rate_signal.units,
